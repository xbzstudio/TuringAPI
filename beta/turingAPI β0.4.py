...
class task():
       ...
       def __init__(self,func,name=None,args=[],kwargs={},returnFunc=None,event=None,association=None):
              self.func=func
              self.name=name
              self.args=args
              self.kwargs=kwargs
       ...
       async def runTask(self):        #运行任务
              ...
              if self.association != None:
                     await loop.run_in_executor(None,self.waitForDone)
              returned = await loop.run_in_executor(None,self.runFunc)
              if self.name != None:
                     if self.returnFunc != None:
                            self.event.returns[self.name]=await self.returnFunc(returned)  #把经过处理的任务函数返回值，放入所属事件流的返回字典中
                     else:
...
def addTask(event,func,name=None,args=[],kwargs={},returnFunc=None,association=None):
       event.tasks.append(task(func,name,args,kwargs,returnFunc,event,association))
